...
select case when (select count(*) 
                  from store_sales 
                  where ss_quantity between 1 and 20) > 572190
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 21 and 40) > 367558
            then (select avg(ss_ext_sales_price)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 61 and 80) > 50345
            then (select avg(ss_ext_sales_price)
                  from store_sales
...
select  
 ...
 where
  ...
  ca_county in ('Montgomery County','Hutchinson County','Pershing County','Lamoille County','Franklin County') and
...
where 
	ws_item_sk = i_item_sk 
  	...
  	and ws_sold_date_sk = d_date_sk
	and d_date between cast('2001-02-20' as date) 
...
select avg(ss_quantity)
 ...
 and((ss_addr_sk = ca_address_sk
  ...
  and ca_state in ('NE', 'NC', 'CA')
  and ss_net_profit between 100 and 200  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('KS', 'WA', 'WV')
  and ss_net_profit between 150 and 300  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('UT', 'TX', 'VA')
...
select  i_item_id
 ...
 where cs_item_sk = i_item_sk 
   ...
   and cs_sold_date_sk = d_date_sk
 and d_date between cast('1998-02-07' as date) 
 				and (cast('1998-02-07' as date) + INTERVAL '30 days')
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
...
with frequent_ss_items as
 ...
 from (select c_last_name,c_first_name,sum(cs_quantity*cs_list_price) sales
      ...
      select c_last_name,c_first_name,sum(ws_quantity*ws_list_price) sales
       ...
       group by c_last_name,c_first_name) AS temp
...
and d_date between '2002-01-27' and 
        (cast('2002-01-27' as date) + INTERVAL '90 days')
...
select  
   w_state
  ,i_item_id
  ,sum(case when (cast(d_date as date) < cast ('2001-04-26' as date)) 
...
select  c_last_name
 ...
 from
   (select ss_ticket_number
    ...
    and store.s_city in ('Riverside','Five Points','Midway','Mount Pleasant','Fairview') 
...
with ss as (
 ...
 where i_item_id in (select
     i_item_id
from item
where i_color in ('puff','saddle','yellow'))
 and     ss_item_sk              = i_item_sk
 and     ss_sold_date_sk         = d_date_sk
 and     d_year                  = 2002
 and     d_moy                   = 6
...
cross_sales as
 (select i_product_name product_name
  ...
  WHERE  ss_store_sk = s_store_sk AND
         ...
         i_color in ('ivory','white','firebrick','frosted','navajo','lace') and
...
select   
 ...
 from (
 	...
 	and ws_ship_mode_sk = sm_ship_mode_sk
        and d_year = 1999
 	and t_time between 56687 and 56687+28800 
...
select c_last_name
   ...
   (select ss_ticket_number
    ...
    and date_dim.d_year in (2000,2000+1,2000+2)
    and store.s_county in ('Ziebach County','Walker County','Williamson County','Walker County')
...
with ss as
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between cast('2001-08-20' as date) 
                  and (cast('2001-08-20' as date) +  INTERVAL '30 days')
       and ss_store_sk = s_store_sk
 group by s_store_sk)
 ,
 sr as
...
select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast('2002-03-14' as date) and (cast('2002-03-14' as date) +  INTERVAL '60 days')
 and i_manufact_id in (73,757,48,745)
 and inv_quantity_on_hand between 100 and 500
 and ss_item_sk = i_item_sk
 group by i_item_id,i_item_desc,i_current_price
 order by i_item_id
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in ('2000-06-06','2000-09-29','2000-11-10')))
...
select  c_customer_id as customer_id
 ...
 where ca_city	        =  'Jackson'
   and c_current_addr_sk = ca_address_sk
   and ib_lower_bound   >=  11489
   and ib_upper_bound   <=  11489 + 50000
...
where ss_item_sk = i_item_sk and
      ...
      d_year in (1999) and
        ((i_category in ('Home','Men','Electronics') and
          i_class in ('lighting','shirts','automotive')
         )
      or (i_category in ('Books','Women','Sports') and
          i_class in ('romance','swimwear','tennis') 
        ))
...
and d_date between '1998-02-26' and 
        (cast('1998-02-26' as date) + INTERVAL '90 days')
...
where 
	ss_item_sk = i_item_sk 
  	and i_category in ('Men', 'Books', 'Music')
  	and ss_sold_date_sk = d_date_sk
