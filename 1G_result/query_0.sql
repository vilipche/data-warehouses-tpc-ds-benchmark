...
select  
  ...
  ,sm_type
  ,web_name
  ,sum(case when (ws_ship_date_sk - ws_sold_date_sk <= 30 ) then 1 else 0 end)  as "30 " 
  ,sum(case when (ws_ship_date_sk - ws_sold_date_sk > 30) and 
                 (ws_ship_date_sk - ws_sold_date_sk <= 60) then 1 else 0 end )  as "31-60 " 
  ,sum(case when (ws_ship_date_sk - ws_sold_date_sk > 60) and 
                 (ws_ship_date_sk - ws_sold_date_sk <= 90) then 1 else 0 end)  as "61-90 " 
  ,sum(case when (ws_ship_date_sk - ws_sold_date_sk > 90) and
                 (ws_ship_date_sk - ws_sold_date_sk <= 120) then 1 else 0 end)  as "91-120 " 
  ,sum(case when (ws_ship_date_sk - ws_sold_date_sk  > 120) then 1 else 0 end)  as ">120 " 
from
   web_sales
  ,warehouse
  ,ship_mode
...
select  i_item_id
 ...
 and d_date between cast('2001-06-09' as date) and (cast('2001-06-09' as date) +  60 )
...
where 
	...
	and d_date between cast('2000-01-05' as date) 
				and (cast('2000-01-05' as date) + 30 )
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast('1998-06-06' as date) and (cast('1998-06-06' as date) +  60 )
...
select  
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast ('2002-05-18' as date) - 30 )
                and (cast ('2002-05-18' as date) + 30 ) 
 group by
    w_state,i_item_id
